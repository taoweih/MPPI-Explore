...
Extracts the rollouts axes region from archive/visualize/u_point_mass/rollouts_step_*.png,
...
and saves clean panels to the senior-thesis plots directory.

Usage:
    python3 -m benchmark.senior_thesis.scripts.u_point_rollouts_vis_util
"""
...
from benchmark.common.paths import archived_visualize_dir, plots_dir

# ── Configuration ─────────────────────────────────────────────────────────

DEFAULT_VIS_DIR = archived_visualize_dir("u_point_mass")
...
def main():
    ...
    parser.add_argument("--input-dir", type=Path, default=DEFAULT_VIS_DIR)
    ...
    for step in STEPS:
        src = args.input_dir / f"rollouts_step_{step:05d}.png"
        if not src.exists():
