"""Ant KNN-k sensitivity benchmark for paper experiments."""
...
RECORD_VIDEO = False
OUTPUT_NAME = "ant_knn_k_sensitivity"
...
def _base_mppi_kwargs(task: Ant) -> dict:
    return dict(
        task=task,
...
def _mppi_factory(task: Ant) -> MPPI:
    return MPPI(**_base_mppi_kwargs(task))


def _dial_factory(task: Ant) -> DIALMPC:
    return DIALMPC(
