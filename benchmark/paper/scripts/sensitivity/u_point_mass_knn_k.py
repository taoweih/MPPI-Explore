"""UPointMass KNN-k sensitivity benchmark for paper experiments."""
...
RECORD_VIDEO = False
OUTPUT_NAME = "u_point_mass_knn_k_sensitivity"
...
def _base_mppi_kwargs(task: UPointMass) -> dict:
    return dict(
        task=task,
...
def _mppi_factory(task: UPointMass) -> MPPI:
    return MPPI(**_base_mppi_kwargs(task))


def _dial_factory(task: UPointMass) -> DIALMPC:
    return DIALMPC(
