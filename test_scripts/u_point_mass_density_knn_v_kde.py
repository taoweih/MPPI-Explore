"""UPointMass benchmark comparing MPPI and density-estimator variants.

This is a focused test script: it reuses the senior-thesis benchmark runner so
success rate, time-to-success, and frequency are computed and saved the same way
as the main benchmark/sensitivity scripts.
"""
...
# Horizon sweep
HORIZONS = np.linspace(0.2, 3.0, 40)
NUM_SAMPLES_FOR_HORIZON_SWEEP = 512

# Sample-count sweep
NUM_SAMPLES_LIST = np.linspace(32, 1024, 40, dtype=int).tolist()
HORIZON_FOR_SAMPLE_SWEEP = 1.3
...
# Output
RECORD_VIDEO = False
OUTPUT_NAME = "u_point_mass_mppi_vs_density_kde_vs_knn_vs_knn_task_state"
OUTPUT_ROOT = Path(__file__).resolve().parent / "tmp_data"

# Parallelism: "sequential", "controllers", "axis", or "all"
PARALLEL = "all"
MAX_WORKERS = "10"  # int or "auto" (= total jobs in batch)
NUM_GPUS = 1
...
def _base_mppi_kwargs(task: UPointMass, horizon: float, num_samples: Optional[int]) -> dict:
    return dict(
        task=task,
...
class TestScriptBenchmarkSuite(SeniorThesisBenchmarkSuite):
    """Benchmark suite that writes outputs under test_scripts/tmp_data."""
    ...
    def _save_results(
        ...
        *,
        ...
        out_dir = OUTPUT_ROOT / f"{OUTPUT_NAME}_{timestamp}"
...
def main() -> None:
    suite = TestScriptBenchmarkSuite(
        ...
        config=SweepConfig(
            ...
            controller_params={
                ...
                "knn_task_state_density": {
                    ...
                    "include_task_state": True,
                    "task_state_weight": KNN_TASK_STATE_WEIGHT,
                    "task_state_dim": UPointMass.state_dim,
                    "position_weight": KNN_POSITION_WEIGHT,
                    "angle_weight": KNN_ANGLE_WEIGHT,
                    "linear_velocity_weight": KNN_LINEAR_VELOCITY_WEIGHT,
                    "angular_velocity_weight": KNN_ANGULAR_VELOCITY_WEIGHT,
